const users = [
    { type: "driver", value: "Maintenances" },
    { type: "button", value: "+ Add Maintenance" },
];

const cols = [
    { type: "ID", value: "ID" },
    { type: "MID", value: "Maintenainer ID" },
    { type: "names", value: "VehicleID" },
    { type: "surname", value: "Service Type" },
    { type: "surname", value: "TotalCost" },
    { type: "phone", value: "Date" },
...
cols.forEach((block) => {
    ...
    if (block.type === "short") {
        ...
        html = `<div style="width: 140px; margin-right:10px;"class="text">
                    <p>${block.value}</p>
            </div>`;
    }
...
function button(block) {
    return `
    <button type="button" class="add-button" onclick="location.href = '/form_maintening.html';">
        <p>${block.value}</p>
    </div>
`;
}
